...
export class ChartEditor {
  ...
  onCellMouseEnter(cellIndex: number): void {
    if (
      this.isEditing() &&
      this.appService.isMouseDown() &&
      this.activeActions().length &&
      !this.cells()[cellIndex].actions.length
    ) {
  ...
  onCellClicked(cellIndex: number): void {
    if (
      this.isEditing() &&
      this.activeActions().length &&
      !this.isActionsEqual(this.activeActions(), this.cells()[cellIndex].actions)
    ) {
      this.updateCell(cellIndex);
    }
  }
  ...
  isActionsEqual(
    ...
    previousActions: ChartEditorCellZoneActionInterface[],
  ): boolean {
    ...
    return actions.every(
      (action, index) =>
        action.type === previousActions[index].type &&
        action.frequency === previousActions[index].frequency,
    );
  }
}
